...
def _make_provider(config: Config):
    ...
    if provider_name == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.amberclaw/config.json under providers.azure_openai section")
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from amberclaw.config.loader import load_config, set_config_path
...
def gateway(
    ...
    from amberclaw.heartbeat.service import HeartbeatService
    ...
    console.print(f"{__logo__} Starting AmberClaw gateway on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show amberclaw runtime logs during chat"),
):
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]AmberClaw is thinking...[/dim]", spinner="dots")
